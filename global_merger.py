...
def merge_data():
    ...
    encoding_latin = ['latin-1', 'latin-1', 'latin-1', 'utf-8', 'utf-8', 'utf-8', 'utf-8', 'utf-8', 'utf-8']

    dfs = []
    for i,j in zip(season_latin, encoding_latin):
        data = pd.read_csv(import_merged_gw(season=f'{i}'), encoding=f'{j}', on_bad_lines="skip")
        data['season'] = i
        dfs.append(data)
        print(f"Loaded data for season {i} with {len(data)} rows.")
